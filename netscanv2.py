...
class NetworkScanner:
    ...
    def _load_oui_cache(self):
        """Load MAC vendor data from local JSON cache"""
        cache_path = Path("data/oui_cache.json")
        try:
            with open(cache_path, 'r', encoding='utf-8') as f:  # Add encoding parameter
                return json.load(f)
        except FileNotFoundError:
            print(f"⚠️ OUI cache not found at {cache_path}", file=sys.stderr)
            return {}
        except json.JSONDecodeError:
            print(f"⚠️ Invalid OUI cache format at {cache_path}", file=sys.stderr)
            return {}
